...
def append_row_with_headers(record: dict) -> bool:
    """
    Append a row to the sheet using HEADERS order. Gracefully handles API errors (429).
    ...
    try:
        ...
        # gspread raises APIError on 429; we log and return False to let caller stop gracefully
        print(f"Error: append_to_sheets_only error: {repr(e)}")
        return False
...
def find_pending_records() -> List[Tuple[int, Dict]]:
    try:
        ...
        print(f"Error reading sheet for pending records: {repr(e)}")
        return []
...
def find_latest_signal_row(symbol: str):
    try:
        ...
        print(f"Error in find_latest_signal_row: {repr(e)}")
        return None
...
def update_cells_by_header(row_idx: int, updates: dict):
    try:
        ...
        print(f"Error updating sheet cells: {repr(e)}")
        return False
